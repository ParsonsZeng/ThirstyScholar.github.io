...
parser.add_argument('file_name', help='file name to save to', type=str)
parser.add_argument('--file_index', help='file index to store result', type=int)
...
for i_episode in range(NUM_EP):
    ...
    while True:
        ...
        if len(traj['s']) == 32:
            ...
            # logger.info(np.array(traj['s']).shape)
            # logger.info(np.array(traj['a']).shape)
            # logger.info(np.array(traj['r']).shape)
...
import pickle
i = args.file_index if args.file_index is not None else 0

file_name = '{0}_{1}.pkl'.format(args.file_name, i)
