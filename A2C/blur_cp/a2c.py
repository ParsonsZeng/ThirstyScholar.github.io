...
for i_episode in range(600):
    ...
    while True:
        ...
        if len(s_lst) == 20 or done:
            ...
            V_ = agent.forward(S_)[0].data.numpy()[0][0]
            ...
            n_step.reverse()
            n_step = np.array(n_step)[:, np.newaxis]
            ...
            loss = -Adv * torch.log(A).gather(1, Act) + c * (V - N_step) ** 2
...
import pickle
with open('a2c_blur_cp_{0}.pkl'.format(args.file_index), 'wb') as handler:
